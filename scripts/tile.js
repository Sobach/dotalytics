  ...
  function tile() {
    ...
    function tile() {
      var z = Math.max(Math.log(scale) / Math.LN2 - 8, 0),
          ...
          y = ty - scale / 2,
          tiles = [],
          cols = d3Array.range(Math.max(0, Math.floor((x0 - x) / k)), Math.min(j, Math.ceil((x1 - x) / k))),
          rows = d3Array.range(Math.max(0, Math.floor((y0 - y) / k)), Math.min(j, Math.ceil((y1 - y) / k)));
